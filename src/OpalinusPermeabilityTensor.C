...
OpalinusPermeabilityTensor::validParams()
{
  ...
  params.addParam<FunctionName>(
      "permeability_tensor_prefactor","1.0",
      "Optional function to use as a scalar prefactor on the permeability tensor.");
  params.addParam<Real>("dip_direction", 0.0, "clock_wise rotation around z-axis. The y-axis is assumed to point NORTH");
  params.addParam<Real>("dip", 0.0, "counter clock-wise rotation around x-axis. The y-axis is assumed to point NORTH");
  params.addRequiredParam<Real>("permeability_parallel", "Permeability of P-sample (flow parallel to bedding), unit m2");
  params.addRequiredParam<Real>("permeability_normal", "Permeability S-sample (loading normal to bedding), unit m2");
  return params;
}

OpalinusPermeabilityTensor::OpalinusPermeabilityTensor(
  ...
  : Material(parameters),
    ...
    _geological_angles(getParam<Real>("dip_direction"),
                       getParam<Real>("dip"))
{
    ...
    const Real s2 = std::sin(Phi);
     ...
     _input_permeability=RealTensorValue(_unrotated_permeability(0,0),_unrotated_permeability(0,1),_unrotated_permeability(0,2),
                                         _unrotated_permeability(1,0),_unrotated_permeability(1,1),_unrotated_permeability(1,2),
                                         _unrotated_permeability(2,0),_unrotated_permeability(2,1),_unrotated_permeability(2,2));
}
